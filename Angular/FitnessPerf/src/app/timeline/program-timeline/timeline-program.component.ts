...
export class TimelineProgramComponent extends TimelineAbstractComponent implements OnInit, OnDestroy {
  @Input()
  program: Program;
  ...
  @Output()
  nextSessionSuggestion: EventEmitter<{programId: string, nextSession: string}> = new EventEmitter();
  ...
  ngOnInit(): void {
    this.sessionMapSubscription = this.sessionService.sessionMapSubject
      .subscribe(() => {
        ...
        for (let sessions of this.sessionsOfThisProgram.values()){
          if (sessions) {
            for (let session of sessions) {
              ...
              let dayWithSession: TimelineDay = this.timelineDays.find(day => Utils.isSameDay(day.date, session.timestamp));
              ...
              let existing = this.timelineDays.filter(timelineDay => {
                let sessionExisting = timelineDay.sessions?.filter(s => s.workout.name === session.workout.name && Utils.isSameDay(s.timestamp, session.timestamp)).length;
