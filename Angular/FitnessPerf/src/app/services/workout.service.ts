...
export class WorkoutService extends BaseService {
      ...
      .pipe(
        map((allWorkoutElement: any) => {
          allWorkoutElement.forEach((element) => {
            ...
            const workoutIndex = this.configuredWorkoutList.findIndex(
              (e) => e.name === workoutName
            );

            if (workoutIndex === -1) {
              this.configuredWorkoutList.push({
                name: workoutName,
                exercises: element[workoutName]
              });
            } else {
              this.configuredWorkoutList[workoutIndex] = {
                name: workoutName,
                exercises: element[workoutName]
              };
            }
  ...
  updateUserExercises(workoutName: string, exerciseList): void {
    ...
    this.getUserDataDocuments().collection(Constants.USER_EXERCISES).doc(workoutName).set(e);
  ...
  addWorkout(newWorkoutName: string, exerciseList?: string[]) {
    let newWorkout: Workout = {
      name: newWorkoutName,
      exercises: (exerciseList ? exerciseList : []) as any[]
    };
