...
export class ConfigurationComponent {
    ...
    fileReader.onload = (e) => {
      ...
      for (let line of parsed) {
        ...
        if (this.isMyJsonCorrect(sessionData)) {
          ...
          this.storageService
            .saveImportedSession(session)
            .then(() => {
              successfulImport++;
            })
            .catch((res) => {
              this.snackbarService.openSnackBar(
                "Erreur lors de l'importation de la séance n° " + lineIndex
              );
              console.error(res);
            })
            .finally(() => {
              this.snackbarService.openSnackBar(
                'Import réussi : ' + successfulImport + '/' + lineIndex + '.\n'
              );
            });
        } else {
  ...
  shallAddToExerciseList(lastWorkoutName: string, session: Session) {
    ...
    if (lastWorkoutName && session.workout.name != lastWorkoutName) {
      this.workoutService.updateUserExercises(
        session.workout.name,
        session.workout.exercises.map((e) => e.name)
      );
    }
  ...
  isMyJsonCorrect(oneJsonSession: any) {
    ...
    let tmpSession: Session = {
      timestamp: oneJsonSession.timestamp,
      workout: oneJsonSession.workout,
      totalLifted: oneJsonSession.totalLifted
    };
