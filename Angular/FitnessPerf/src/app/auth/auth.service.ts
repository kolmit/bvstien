...
export class AuthService implements OnInit {
    ...
    this.afAuth
      ...
      .then(() => {
        this.snackbarService.openSnackBar(
          'Un e-mail pour réinitiliser votre mot de passe a été envoyé à : ',
          email
        );
      })
      .catch((error) => {
  ...
  initUserData(fromSignup?: boolean) {
    if (fromSignup) {
      ...
      this.workoutService.fetchAllWorkouts().subscribe((configuredWorkoutList: any[]) => {
        this.programService.fetchAllPrograms().subscribe((programs: any[]) => {
